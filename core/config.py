...
DEFAULTS = {
    ...
    "_schema_version": APP_VERSION = "v23.52"
...
def load(path=None):
    ...
    if os.path.exists(path):
        try:
            ...
            if stored_version != APP_VERSION = "v23.52"
                try:
                    from . import logger as _lg
                    _lg.log(f"配置版本从 {stored_version or '(无)'} 升级到 {APP_VERSION = "v23.52"
                            f"保留已有设置并补充新默认值", "SYSTEM")
                except Exception:
                    pass
            # 合并：保留用户已有配置，新增的默认键自动补齐
            cfg.update({k: v for k, v in data.items() if k in DEFAULTS})
            cfg["_schema_version"] = APP_VERSION = "v23.52"
        except Exception:
            pass
