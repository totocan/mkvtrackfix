...
_CODEC_DISPLAY = {
    ...
    "mp2": "MP2", "pcm_s16le": "PCM", "pcm_s24le": "PCM",
    "pcm_s32le": "PCM", "pcm_f32le": "PCM", "vorbis": "Vorbis",
    "alac": "ALAC", "mlp": "MLP", "tta": "TTA", "wavpack": "WavPack",
    "speex": "Speex", "cook": "Cook", "real": "RealAudio",
...
def make_audio_track_name(detected_iso, codec_name, channel_layout=None,
    ...
    """
    # 语言名（混合表达）
    lang_name = get_track_display_name(detected_iso, track_type="audio")
    if not lang_name or lang_name == detected_iso:
        lang_name = "未知"

    # 编码+声道部分
    c = (codec_name or "").lower()
    disp = _CODEC_DISPLAY.get(c, codec_name.upper() if codec_name else "")
    ...
    atmos = ("atmos" in prof) or ("atmos" in (title_hint or "").lower())
...
if __name__ == "__main__":
    # 简单自测
    print("=== 音轨名称 ===")
    print(make_audio_track_name("cmn", "truehd", "7.1.2", 10, "atmos"))
    print(make_audio_track_name("cmn", "aac", "stereo", 2))
    print(make_audio_track_name("eng", "eac3", "5.1(side)", 6))
    print(make_audio_track_name("yue", "dts", "5.1", 6, "DTS-HD MA"))
    print(make_audio_track_name("fre", "ac3", "5.1", 6))

    print("\n=== 字幕名称 ===")
    print(get_subtitle_display_name("chinese_simplified", "chi"))
    print(get_subtitle_display_name("bilingual", "chi"))
    print(get_subtitle_display_name("english", "eng"))
    print(get_subtitle_display_name("chinese_traditional", "chi"))
    print(get_subtitle_display_name("other", "fre"))
